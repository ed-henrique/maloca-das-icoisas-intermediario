...
def get_pacientes():
    try:
        ...
        doc_ref = db.collection('pacientes').document(str(paciente_id)).collection('dados')
        doc = doc_ref.get()

        periodo_inicio_novo = utc.localize(datetime.fromisoformat(periodo_inicio))
        ...
        for record in doc:
            ...
            if isinstance(ts, datetime) and periodo_inicio_novo <= ts <= periodo_fim_novo:
                filtered.append(record.to_dict())  # Converte timestamp para string ISO
